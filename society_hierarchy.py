...
class Citizen:
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        if not self._subordinates:
            ...
            subs = []
            for s in self._subordinates:
                subs += s.get_all_subordinates()
            return merge(self._subordinates, subs)
...
class Society:
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if not self._head:
            ...
            is_citizen = self._head.get_citizen(cid)
            if is_citizen:
                return is_citizen
    ...
    def get_all_citizens(self) -> List[Citizen]:
        ...
        True
        """
        all_subs = self._head.get_all_subordinates()
        return merge([self._head], all_subs)
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if not superior_id:
            ...
            # if superior_id exists, there is at least one citizen who has cid
            # equal to superior_id
            sup = self._head.get_citizen(superior_id)
            if not sup:
                if self._head.cid == superior_id:
                    self._head.add_subordinate(citizen)
                    citizen.set_superior(self._head)
            else:
                sup.add_subordinate(citizen)
                citizen.set_superior(sup)
    ...
    def get_citizens_with_job(self, job: str) -> List[Citizen]:
        ...
        citizens_with_this_job = []
        for c in all_citizens:
            if c.job == job:
                citizens_with_this_job.append(c)
        return citizens_with_this_job
    ...
    def change_citizen_type(self, cid: int,
        ...
        if not citizen:
            if self._head.cid == cid:
                ...
                print('THIS CITIZEN DOES NOT EXIST WHICH CAN"T BE HAPPENING')
        ...
        if citizen_found:
            ...
            elif isinstance(citizen, Citizen):
                new_cz = DistrictLeader(
                    citizen.cid,
                    citizen.manufacturer,
                    citizen.model_year,
                    citizen.job,
            ...
            # Re-link all subordinates and superior from old DL to new CZ
            citizen_subs = citizen.get_direct_subordinates()
            citizen_sup = citizen.get_superior()
            self.delete_citizen(cid)
            if citizen_sup:
                for s in citizen_subs:
                    citizen_sup.remove_subordinate(s.cid)
                    s.set_superior(new_cz)
                    new_cz.add_subordinate(s)
                self.add_citizen(new_cz, citizen_sup.cid)
            else:
                new_cz.set_superior(None)
                self.add_citizen(new_cz)
            return new_cz
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        if citizen.rating >= sup.rating:
            sup = citizen.get_superior()
            sup.remove_subordinate(citizen.cid)
            # citizen sup is now set to none, now we set it to new superior
            citizen.become_subordinate_to(sup.get_superior())
            # citizen.set_superior(sup.get_superior())
            # now we set all of sup's subordinates' superior to citizen and
            # set all subordinate to
            sup_sub = sup.get_direct_subordinates()[:]
            for s in citizen.get_direct_subordinates():
                s.become_subordinate_to(sup)
            for s in sup_sub:
                s.become_subordinate_to(citizen)
            sup.become_subordinate_to(citizen)

            if isinstance(sup, DistrictLeader):
                sup_district = sup.get_district_name()
                self.change_citizen_type(sup.cid)
                new_cz = self.change_citizen_type(citizen.cid, sup_district)
                return new_cz
        return citizen
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        if citizen.cid == self._head.cid:
            if not citizen.get_direct_subordinates():
                self._head = None
            else:
                highest_rated = citizen.get_highest_rated_subordinate()
                for s in citizen.get_direct_subordinates():
                    s.set_superior(highest_rated)
                    highest_rated.add_subordinate(s)
                    citizen.remove_subordinate(s.cid)
                highest_rated.set_superior(None)
                highest_rated.remove_subordinate(highest_rated.cid)
                citizen.get_superior().remove_subordinate(citizen.cid)
        else:
            if not citizen.get_direct_subordinates():
                # just remove from sup's sub
                citizen.get_superior().remove_subordinate(citizen.cid)
            else:
                sup = citizen.get_superior()
                for s in citizen.get_direct_subordinates():
...
class DistrictLeader(Citizen):
    ...
    def get_district_citizens(self) -> List[Citizen]:
        ...
        >> > c4 = Citizen(4, "S.T.A.R.R.Y Lab", 3010, "Labourer", 55)
        >> > c2.become_subordinate_to(c1)
        >> > c3.become_subordinate_to(c1)
        >> > c4.become_subordinate_to(c2)
        >> > c1.get_district_citizens() == [c1, c2, c3, c4]
        True
        """
...
def district_society_demo() -> Society:
    ...
    """
    c = DistrictLeader(6, "Starky Industries", 3036,
                       "Commander", 50, "Area 52")
